...
import { APP_ID as DEFAULT_APP_ID, ALLOWED_USERS as DEFAULT_ALLOWED_USERS, MAIN_SESSION_ID, MAIN_SESSION_FILE, WS_JSON, LAST_ACTIVE_FILE, WORKDIR, HOME, PLUGIN_DIR } from './config.js'; // 本地配置（脱敏边界）
...
class DSHFeishuBot {
  ...
  async semanticLeafRoute(text, category, currentId = null) {
    const facts = extractKeyFacts(text);
    // 会话映射表优先（用户裁定: 无明确新任务先查映射表）——tag/desc 是策展身份卡, 比首条消息碎片稳定
    const allLeaves = this.sessionRegistry.sessionMap()
      .filter((s) => s.status !== 'archived')
      .map((s) => [s.id, s]);
    // 强树干用同树干候选; 弱树干（chat/query 兜底, 树干信息量为零）或同树干为空 → 恒全量
    // （2026-09-14 仿真测试发现: 弱树干下只要 chat 树有任意叶, 他树干真候选就被截断）
    const weakBucket = category === 'chat' || category === 'query';
    const sameL2 = allLeaves.filter(([, s]) => s.path?.includes(category));
    const candidates = (weakBucket || !sameL2.length) ? allLeaves : sameL2;
    ...
    // 打分取 max(原始指纹, 身份卡 tag/desc 指纹)——取并集会稀释命中率（分母变大）;
    // 另设身份卡强命中: 策展卡的任一词被子串/2-gram 命中即算（卡是高精度信息, 单词命中即可信）
    const cardFacts = (s) => [s.tag, ...extractKeyFacts(s.desc || '')].filter(Boolean);
    const cardHit = (s) => cardFacts(s).some((cf) => facts.some((nf) => cf === nf || (nf.length >= 2 && cf.includes(nf)) || nf.includes(cf)));
    let bestId = null, bestScore = 0;
    for (const [sid, s] of candidates) {
      const sc = Math.max(
        matchScore(facts, s.keyFacts || []),
        matchScore(facts, cardFacts(s)));
      if (sc > bestScore) { bestScore = sc; bestId = sid; }
    }
    // 卡强命中 > 树干闸门: 策展卡命中是高精度证据, 可压过正则树干的误判
    // （2026-09-14 仿真 #21: "租约条款"误锚 legal, 海峡(business)身份卡含"租约"——卡命中应当家）
    const cardBest = this.sessionRegistry.sessionMap()
      .filter((s) => s.status !== 'archived')
      .map((s) => [s.id, s])
      .filter(([, s]) => cardHit(s))
      .sort((a, b) => matchScore(facts, cardFacts(b[1])) - matchScore(facts, cardFacts(a[1])))[0];
    if (cardBest) {
      this.logDecision('card-hit', { text: text.slice(0, 40), winner: cardBest[0], card: cardBest[1].tag });
      return { reuse: cardBest[0], score: 0.9, via: 'card' };
    }
    if (bestId && bestScore >= 0.34 && !crossTreeBlocked(bestId)) {
      this.logDecision('fingerprint', { text: text.slice(0, 40), sessionId: bestId, score: +bestScore.toFixed(2) });
      return { reuse: bestId, score: bestScore };
    }
    if (bestId && bestScore >= 0.34) {
      this.log(`🚫 指纹续作被树干闸门拦截: ${bestId} 不含 [${category}] ← ${text.slice(0, 30)}`);
      this.logDecision('gate-block', { text: text.slice(0, 40), sessionId: bestId, category, via: 'fingerprint' });
    }
...
不要输出任何解释。`,
        routeUser)).trim();
      this.logDecision('llm-route', { text: text.slice(0, 40), category, branchCount: candidates.length, promptTail: routeUser.slice(-400), answer: ans.slice(0, 120) });
      ...
      if (hit) {
        this.log(`🚫 LLM 续作被树干闸门拦截: ${hit[0]} 不含 [${category}] ← ${text.slice(0, 30)}`);
        this.logDecision('gate-block', { text: text.slice(0, 40), sessionId: hit[0], category, via: 'llm' });
      }
      return { casual: true };
    } catch (e) {
  ...
  async rotateLeafIfNeeded(sessionId, category) {
    try {
      ...
      this.logDecision('rotate', { from: sessionId, to: newId, events: events.length });
      return newId;
  ...
  // 会话映射表身份卡（2026-09-14 用户裁定）: 建叶定 tag(L3-L4 名), 首轮产出后 LLM 蒸馏 desc(≤40字)
  async ensureSessionTag(sessionId, { topic, workResult } = {}) {
    try {
      const rec = this.sessionRegistry.get(sessionId);
      if (!rec) return;
      if (!rec.tag && rec.path?.length >= 4) {
        const meta = this.sessionRegistry.leafSessionMeta(rec.path);
        this.sessionRegistry.register(sessionId, { tag: meta.sessionId });
        rec.tag = meta.sessionId;
      }
      if (!rec.desc && workResult) {
        const desc = (await chatText(
          '给这个工作会话写一句身份描述（≤40字, 格式: 对象+事项范围, 如"福州仓山奥体全季酒店投资尽调: 报告/清单/租约/估值"）。只输出描述本身, 不要解释。',
          `会话主题: ${topic || rec.tag || sessionId}\n首轮产出摘要: ${String(workResult).slice(0, 300)}`)).trim().replace(/["'\n]/g, '').slice(0, 48);
        if (desc) this.sessionRegistry.register(sessionId, { desc });
      }
    } catch (e) { this.log('身份卡生成失败(忽略):', e.message); }
  }
  ...
  async routeWorkTarget(text, { forceNew = false } = {}) {
    ...
    if (!forceNew) {
      ...
      if (!workTarget) {
        ...
        if (d.reuse) {
          ...
          this.logDecision('new-leaf', { text: text.slice(0, 40), topic: [d.newDomain, d.newTopic].filter(Boolean).join('-'), l2: effectiveL2, path: leafPath.join('/') });
          workTarget = await this.resolveLeafAgent(leafPath, text, regexCategory);
        }
      }
    ...
    if (!workTarget) {
      workTarget = await this.resolveCategoryAgent(regexCategory, forceNew, null);
      if (workTarget) this.logDecision('l2-fallback', { text: text.slice(0, 40), category: regexCategory, sessionId: workTarget.sessionId });
    }
  ...
  async handleCompoundMessage(chatId, text, items) {
    ...
    this.logDecision('compound', { text: text.slice(0, 60), items: items.length, itemTexts: items.map((x) => x.slice(0, 20)) });
  ...
  // 语义决策日志（2026-09-14 用户裁定）: routing-decisions.jsonl —— 让每次路由判定可审计
  // （此前 LLM 判定是黑盒, 排查误判只能靠猜）; 超 5MB 自动截尾保留 2000 行
  logDecision(type, data = {}) {
    try {
      const file = `${PLUGIN_DIR}/routing-decisions.jsonl`;
  ...
  async handleMessage(msg) {
    ...
    if (explicitTopic) {
      this.log(`🎯 显式项目名词命中树叶路径 ${formatPath(prePath)}, 跳过延续判定 ← 「${text.slice(0, 30)}」`);
      this.logDecision('explicit', { text: text.slice(0, 40), category: regexCategory, path: prePath.join('/') });
    }
    ...
    if (current) {
      ...
      try {
        if (quickContinue) {
          ...
          this.logDecision('continue-quick', { text: text.slice(0, 40), sessionId: current.sessionId });
        } else {
          ...
          if (overlap >= 0.34) {
            ...
            this.logDecision('continue-fingerprint', { text: text.slice(0, 40), sessionId: current.sessionId, score: +overlap.toFixed(2) });
          } else {
            ...
            if (/^无关/.test(ans)) {
              ...
              this.logDecision('relatedness', { text: text.slice(0, 40), topic, answer: '无关', continued: false });
              // 不置 workTarget, 落入下方正常路由
            } else {
              await continueCurrent('关联判定「相关」');
              this.logDecision('relatedness', { text: text.slice(0, 40), topic, answer: ans.slice(0, 20), continued: true });
            }
          }
        }
    ...
    // 5) 会话映射表身份卡（tag/desc, 无 desc 时本轮蒸馏）
    if (workTarget.path?.length >= 4) this.ensureSessionTag(workTarget.sessionId, { topic: text.slice(0, 30), workResult }).catch(() => {});
  }
