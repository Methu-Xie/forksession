...
export const L34_RULES = [
  ...
  { node: 'forksession',      patterns: [/forksession|forknsession|forksesion|forksessoin|fork[-\s]?session|fork\s?会话|fork\s?分支|分支会话机制|root[-\s]?branch/i] }, // 含常见拼写变体（forknsession 等）
...
export class SessionRegistry {
  ...
  register(id, { path, parent, origin = 'feishu', note, keyFacts, status, tag, desc } = {}) {
    this.load();
    const now = Date.now();
    ...
    this.data.sessions[id] = {
      ...
      ...(tag ? { tag } : prev.tag ? { tag: prev.tag } : {}),
      ...(desc ? { desc } : prev.desc ? { desc: prev.desc } : {}),
    };
    this.save();
  ...
  /** 会话映射表（2026-09-14 用户裁定: 分类优先查映射表）:
   *  每个会话一行简短身份卡 { id, tag, desc, path, status, lastActiveAt },
   *  路由候选与 /tree 展示共用。tag=简短标识, desc=一句话范围描述（LLM 蒸馏, 可手动钉死） */
  sessionMap() {
    return Object.entries(this.data.sessions)
      .filter(([, s]) => s.origin !== 'web')
      .map(([id, s]) => ({
        id,
        tag: s.tag || id,
        desc: s.desc || s.note || '',
